...
case "${DISTRO}" in
    ...
    anolis)
        BUILD_RPM=1
        cp -f anolis-23/Dockerfile "${TMPDIR}/rpm/fedora-43/Dockerfile"
        ;;
    opencloudos)
        BUILD_RPM=1
        cp -f opencloudos-23/Dockerfile "${TMPDIR}/rpm/fedora-43/Dockerfile"
        ;;
    *)
...
if [ "${BUILD_RPM}" = '1' ]; then
    make ARCH=loong64 ARCHES=loong64 VERSION=${VERSION} REF=${REF} DOCKER_ENGINE_REF=docker-${REF} GO_VERSION=${GO_VERSION} GO_IMAGE=${GO_IMAGE} fedora-43
fi
...
if [ "${BUILD_RPM}" = '1' ]; then
    mv ${TMPDIR}/rpm/rpmbuild/fedora-43/* dist/
fi
